...
void init( int node, int i, int j ) {
    if ( i == j ) {
        ...
        init( node<<1, i, ( i + j ) / 2 );
        init( node<<1|1, ( i + j ) / 2 + 1, j );
        no left = tree[ node<<1], right = tree[ node<<1|1 ];
...
no query( int node, int a, int b, int i, int j ) {
    if ( a == i && b == j ) {
        ...
        return query( node<<1, a, ( a + b ) / 2, i, j );
    }
    if ( i > ( a + b ) / 2 ) {
        return query( node<<1|1, ( a + b ) / 2 + 1, b, i, j );
    }
    no left = query( node<<1, a, ( a + b ) / 2, i, ( a + b ) / 2 );
    no right = query( node<<1|1, ( a + b ) / 2 + 1, b, ( a + b ) / 2 + 1, j );
    return ( ( no ) {
